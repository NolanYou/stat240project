...
In our calculations, we assume that the data is well sampled, and thus the sample data should be normally distributed due to the central limit theorem because n > 30. However, it is clear that the sample itself is not quite normal, meaning that some assumptions of a normal distribution are violated slightly. This is further touched on in the discussion section.

```{r,  warning = FALSE, echo = FALSE}
data%>%filter(type == "Stroke" & race == "Overall" & sex == "Overall" & age_group == "Ages 35-64 years")%>%
  ggplot(aes(x = data)) + geom_histogram(bins = 30) +
  labs(title = "Histogram of overall stroke mortality rate vs number of counties", subtitle = " for 35-64 year olds per 100,000", x = "Mortality rate", y = "Count of counties")


data%>%filter(type == "CHD" & race == "Overall" & sex == "Overall" & age_group == "Ages 35-64 years")%>%
  ggplot(aes(x = data)) + geom_histogram(bins = 30) + 
  labs(title = "Histogram of overall CHD mortality rate vs number of counties", subtitle = " for 35-64 year olds per 100,000", x = "Mortality rate", y = "Count of counties")

data%>%filter(type == "Stroke" & race == "Overall" & sex == "Overall" & age_group == "Ages 65 years and older")%>%
  ggplot(aes(x = data)) + geom_histogram(bins = 30) +
  labs(title = "Histogram of overall stroke mortality rate vs number of counties", subtitle = "for 65+ year olds per 100,000", x = "Mortality rate", y = "Count of counties")

data%>%filter(type == "CHD" & race == "Overall" & sex == "Overall" & age_group == "Ages 65 years and older")%>%
  ggplot(aes(x = data)) + geom_histogram(bins = 30) +
  labs(title = "Histogram of overall CHD mortality rate vs number of counties", subtitle = "for 65+ year olds per 100,000", x = "Mortality rate", y = "Count of counties")
```
...
age_chd_test
```
There is very strong evidence that the mean mortality rate of those aged 35 to 64 who died from coronary heart disease is smaller than the mean of the mortality rate of those aged 65 and older from heart disease (t-stat = -270.64 on 13037 df, $p = 2.2e-16$, two-sided t-test, unequal variances).
...
age_stroke_test
```
There is very strong evidence that the mean mortality rate of those aged 35 to 64 who died from stroke is smaller than the mean of the mortality rate of those aged 65 and older from heart disease (t-stat = -293.92 on 12741 df, $p = 2.2e-16$, two-sided t-test, unequal variances).
...
sex_chd_test
```
There is very strong evidence that the mean mortality rate of women who died from coronary heart disease is smaller than the mean of the mortality rate of men from heart disease (t-stat = -57.121 on 43777 df, $p = 2.2e-16$, two-sided t-test, unequal variances, extreme test statistic).
