...
The raw data set contains over 5 million rows and 21 columns, but we will only use 677,490 of them, excluding rows with missing information for the data value column and only selecting deaths caused by coronary heart disease (CHD) or stroke^[```{r, include = FALSE}
##Cleaning Data
# data = all_data %>% 
#   select(Year,LocationAbbr, LocationDesc, Topic, Data_Value, Stratification1, Stratification2, Stratification3) %>% 
#   rename(year = Year, state = LocationAbbr, county = LocationDesc, age_group = Stratification1, data = Data_Value, type = Topic, race = Stratification2, sex = Stratification3) %>% 
...
To address RQ2, we will look at the influence sex has on each cause of death and their corresponding mortality rate to asnwer RQ1. The data was filtered to obtain only the sex influenced rows^[```{r, warning = FALSE}
# sex_data= data %>% 
...
Race may also have an influence on the mortality rate between coronary heart disease and stroke. In order to answer RQ3, we will test whether there is any difference between a specific race and the overall average of all. The data was filtered to obtain only the sex influenced rows^[```{r, warning = FALSE}
# race_data= data %>% 
#   filter(type != "Heart failure" & type != 	"Cardiovascular disease (CVD)" & type != "All heart disease", race != "Overall", sex == "Overall") %>% 
#   select(!sex)
# 
