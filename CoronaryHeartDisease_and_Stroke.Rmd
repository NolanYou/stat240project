...
To answer R2, we wanted to determine how geographic location had influenced the rates of coronary heart disease and stroke mortality rates over the past 20 years. In order to do so, the stratifications were ignored and the overall data was used, save for the age which was controlled for by only selecting those aged 35-64. The average change in both CHD and stroke mortality rate per county, per state was then graphed to analyze.
...
There does not appear to be an obvious pattern to either the incidence of stroke fatalities or based on geographic location across the U.S. However it appears same locations that experienced an decrease in fatality rate from strokes had a decrease in coronary heart disease as well. 
