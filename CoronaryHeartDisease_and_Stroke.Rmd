...
To answer RQ2, we wanted to determine how geographic location had influenced the rates of coronary heart disease and stroke mortality rates over the past 20 years. In order to do so, the stratification were ignored and the overall data was used, save for the age which was controlled for by only selecting those aged 35-64^[```{r, warning = FALSE}
...
``` ]. The average change in both CHD and stroke mortality rate per county, per state was then graphed using an imported USA map tool^[https://jtr13.github.io/cc19/different-ways-of-plotting-u-s-map-in-r.html] to analyze the data^[```{r, warning = FALSE} 
...
#      name = "Change in Fatality Rate per 100,000 People", label = scales::comma
#    ) + labs(title = "Average Change per County from 1999 to 2019 in Coronary Heart Disease Fatalities by State") +
...
#   scale_fill_viridis_c(
#     name = "Change per County in Fatality Rate per 100,000", label = scales::comma
#   ) + labs(title = "Average Change per County from 1999 to 2019 in Stroke Fatalities by State") +
#   theme(panel.background = element_rect(color = "black", fill = "lightblue"), legend.position = "right")
```].
...
Based on these graphs, there does not appear to be an obvious pattern to either the incidence of stroke fatalities or based on geographic location across the U.S. However it appears same locations that experienced an decrease in fatality rate from strokes had a decrease in coronary heart disease as well. 
...
There are some shortcomings in our project. First, we are not able to create a generalization for the entire U.S. because certain states are not included in the data, which could be solved if we had data from every U.S. state. Additionally, we may have drawn conclusions predicated on faulty assumptions. For example, such as the reporting of who dies from a CHD or a stroke might actually be biased and non-indicative of the overall parent population of everyone, not random, so we cannot assume that the data is random. Additionally, our data is continuous while there is a discrete number of people dying from diseases, meaning some rounding has to be done.

We would benefit greatly from having full medical records, because there may be other factors influencing CHD, and we could expand our research question more. Some of those other factors we could look at include if the person smokes, prediagnosed conditions, and whether or not that disease is hereditary. 

One further idea that was not completely researched in this paper was how these factors changed over time, which could be futher researched as well.

A different method to address how age, race, gender and geographic location  would be to pool global data, along with controlling for factors that were not controlled in this project, like economic status.
